  ...
  if (p.startsWith('/api/summary/')) {
    ...
    return json(await cached(`sum:${symbol}`, 600_000, async () => {
      ...
      return {
        ...
        high52w: num(sd.fiftyTwoWeekHigh), low52w: num(sd.fiftyTwoWeekLow), avgVolume: num(sd.averageVolume), bid: num(sd.bid), ask: num(sd.ask), bidSize: num(sd.bidSize), askSize: num(sd.askSize),
